...
# ═══════════════════════════════════════════════════════════════════════════════
# 12. HTTP Dependency Detection & Mock Generation Stubs
# ═══════════════════════════════════════════════════════════════════════════════

# Detect HTTP/HTTPS service dependencies from README
# Args: $1 = path to README.md
# Outputs: newline-separated "service:url" pairs for each detected external HTTPS dependency
detect_http_dependencies() {
    local readme_file="$1"
    [[ ! -f "$readme_file" ]] && return 0

    # Extract https:// URLs, exclude localhost/127.0.0.1 and common non-service domains
    local urls
    urls=$(grep -oE 'https?://[a-zA-Z0-9._/-]+' "$readme_file" \
        | grep -v 'localhost' \
        | grep -v '127\.0\.0\.1' \
        | grep -v 'github\.com' \
        | grep -v 'shields\.io' \
        | grep -v 'npmjs\.\(com\|org\)' \
        | grep -v 'pypi\.org' \
        | grep -v 'docs\.' \
    ...
    while IFS= read -r url; do
        ...
        # Derive service name: strip api./www. prefix, take first label
        local service
        service=$(echo "$host" | sed -E 's/^(api|www)\.//' | awk -F. '{print $1}')
        echo "${service}:${url}"
    done <<< "$urls"
}

# Generate mock stub files for detected dependencies
# Args: $1 = output directory, $2 = newline-separated "service:url" dependency list
generate_mock_stubs() {
    local output_dir="$1"
    local deps="$2"
    ...
    while IFS= read -r dep; do
        ...
        local stub_file="${output_dir}/${service}.py"
        cat > "$stub_file" << STUBEOF
#!/usr/bin/env python3
"""Minimal mock server for ${service}"""
import os, sys
...
STUBEOF
        chmod +x "$stub_file"
        # Write port metadata so generate_start_mocks reads the same port
        echo "$port" > "${output_dir}/${service}.port"
        port=$((port + 1))
    done <<< "$deps"
    return 0
}

# Generate start-mocks.sh script
# Scans mocks_dir for *.py and *.sh stub files and generates a launcher that
# starts each stub, writes its PID, and exports MOCK_<SERVICE>_URL env vars.
# Args: $1 = mocks directory
generate_start_mocks() {
    local mocks_dir="$1"
    [[ ! -d "$mocks_dir" ]] && return 1

    local script="${mocks_dir}/start-mocks.sh"
    printf '#!/bin/bash\n# Start mock servers for UAT — auto-generated by buildcrew\n' > "$script"
    printf 'MOCKS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"\n\n' >> "$script"

    for stub_file in "${mocks_dir}"/*.py "${mocks_dir}"/*.sh; do
        [[ -f "$stub_file" ]] || continue
        local svc
        svc=$(basename "$stub_file")
        svc="${svc%.*}"
        [[ "$svc" == "start-mocks" || "$svc" == "stop-mocks" ]] && continue

        local svc_upper
        svc_upper=$(echo "$svc" | tr '[:lower:]' '[:upper:]')

        # Read port from metadata file written by generate_mock_stubs
        local port=9001
        local port_file="${mocks_dir}/${svc}.port"
        if [[ -f "$port_file" ]]; then
            port=$(cat "$port_file")
        fi

        local basename_file
        basename_file=$(basename "$stub_file")
        if [[ "$stub_file" == *.py ]]; then
            printf 'python3 "$MOCKS_DIR/%s" &\n' "$basename_file" >> "$script"
        else
            printf 'bash "$MOCKS_DIR/%s" &\n' "$basename_file" >> "$script"
        fi
        printf 'export MOCK_%s_URL="http://localhost:%s"\n\n' "$svc_upper" "$port" >> "$script"
    done
...
# Generate stop-mocks.sh script
# Kills all mock server processes by reading their PID files.
# Args: $1 = mocks directory
generate_stop_mocks() {
    local mocks_dir="$1"
    [[ ! -d "$mocks_dir" ]] && return 1

    local script="${mocks_dir}/stop-mocks.sh"
    cat > "$script" << 'STOPEOF'
#!/bin/bash
# Stop mock servers for UAT — auto-generated by buildcrew
