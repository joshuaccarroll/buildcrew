...
# Show a brief tip about plugins (for first run)
# Called during first `buildcrew run`
show_plugin_tip() {
    local project_types=$(detect_project_type)

    # Only show tip if there are actual project files (not just git/any)
    local has_project_files=false
    for ptype in $project_types; do
        if [[ "$ptype" != "git" && "$ptype" != "any" ]]; then
    ...
    if [[ "$has_project_files" == "true" ]]; then
        echo -e "${CYAN}Tip:${NC} Run ${CYAN}buildcrew plugins${NC} to see recommended plugins for this project."
        echo ""
    fi
