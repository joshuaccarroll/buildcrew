...
- Each phase is a separate skill: `skills/buildcrew-*/SKILL.md` (spec, research, review, build, simplify, codereview, verify).
...
- **Execution mode** — Autonomous pipeline that processes pending BACKLOG.md tasks through the phase sequence above. Includes inline UAT as a pipeline stage after verify for non-trivial tasks with a `README.md`. Opt out with `--no-uat`.
